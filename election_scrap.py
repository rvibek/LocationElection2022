...
electoralref = pd.merge(localbodies, districts, how='outer', left_on='parentId', right_on='id')
electoralref.columns = ['id', 'GaPa_NaPa', 'districtId', 'districtIdy', 'district', 'stateId', 'state' ]
...
pd.concat([pd.read_csv(file) for file in files]).drop(columns='Unnamed: 0').reset_index().drop(columns='index').to_csv('EC_merged.csv', index=False)
